      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mb-8">
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-3">
            <div className="rounded-2xl bg-white p-5 shadow-soft"><h3 className="font-bold text-gray-900">Recent Giving</h3><div className="mt-4 space-y-3">{overview.sent.length ? overview.sent.map((gift: any) => <Link key={gift.id} href={`/requests/${gift.request.id}`} className="block rounded-xl bg-gray-50 px-3 py-3 text-sm"><span className="flex items-center justify-between gap-3"><span className="line-clamp-1 font-medium text-gray-800">{gift.request.title}</span><span className="shrink-0 font-bold text-[#d6652f]">{formatMoney(gift.amount)}</span></span>{gift.message && <span className="mt-2 block border-l-2 border-primary-300 pl-2 text-xs italic leading-5 text-gray-600">“{gift.message}”</span>}</Link>) : <p className="text-sm text-gray-500">No donations made yet.</p>}</div></div>
            <div className="rounded-2xl bg-white p-5 shadow-soft"><h3 className="font-bold text-gray-900">Recent Support Received</h3><div className="mt-4 space-y-3">{overview.received.length ? overview.received.map((gift: any) => <Link key={gift.id} href={`/requests/${gift.request.id}`} className="block rounded-xl bg-gray-50 px-3 py-3 text-sm"><span className="flex items-center justify-between gap-3"><span className="line-clamp-1 font-medium text-gray-800">{gift.request.title}</span><span className="shrink-0 font-bold text-green-600">{formatMoney(gift.amount)}</span></span>{gift.message && <span className="mt-2 block border-l-2 border-green-300 pl-2 text-xs italic leading-5 text-gray-600">“{gift.message}”</span>}</Link>) : <p className="text-sm text-gray-500">No support received yet.</p>}</div></div>
                    ...
                    <div className="space-y-4">
                      {data.receivedDonations.map((donation: any) => (
                        <div key={donation.id} className="rounded-2xl border border-gray-200 p-4">
                          ...
                          {donation.message && (
                            <p className="mt-3 rounded-xl bg-primary-50 px-3 py-2 text-sm italic leading-6 text-gray-700">
                              “{donation.message}”
                            </p>
                          )}
